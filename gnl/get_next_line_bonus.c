...
/*   Updated: 2023/10/21 12:44:06 by ashigema         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!saved_line[fd])
	{
		saved_line[fd] = ft_strdup("");
		if (!saved_line[fd])
			return (NULL);
	}
	saved_line[fd] = read_until_newline(fd, saved_line[fd]);
	if (!saved_line[fd])
		return (NULL);
	line = extract_line_from_saved(saved_line[fd]);
	if (!line)
		return (NULL);
	saved_line[fd] = trim_saved_line(saved_line[fd], ft_strlen(line));
	if (!saved_line[fd])
		return (NULL);
	return (line);
}
